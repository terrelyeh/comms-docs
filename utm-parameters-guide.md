...
很多人第一次用會搞混「那我為什麼不直接看 GA 就好？」兩套工具的定位其實**完全不重疊** — 一個看「點擊前」，一個看「點擊後」。UTM 是它們的共同語言。

### 一句話總結

- **短網址工具（此平台）** = landing page **之前**的流量分析 → 「誰點了我的連結？從哪點的？」
- **GA4 / HubSpot** = landing page **之後**的行為分析 → 「進站的人做了什麼？有沒有轉換？」

### 分工對照表

| 維度 | 短網址工具（此平台） | GA4 / HubSpot |
|---|---|---|
| **時機** | 使用者**點擊連結那一刻** | 使用者**抵達網站之後** |
| **回答的問題** | 哪條連結、哪個素材、哪個通路帶來最多點擊？ | 進站後看了幾頁？待多久？有沒有註冊 / 下單？ |
| **資料單位** | 一次點擊（click） | 一次工作階段（session）+ 事件（event） |
| **管理對象** | 連結、活動、UTM 規範、QR code、A/B 變體 | 頁面、事件、轉換、使用者屬性 |
| **核心指標** | 點擊數、獨立點擊、地區、裝置、時段、變體勝出率 | 跳出率、停留時間、轉換率、漏斗、LTV |
| **是否需要網站程式碼** | ❌ 不需要，連結掛出去就開始追蹤 | ✅ 需要在網站埋 GA tag / 事件 |
| **能不能追到非自家網站** | ✅ 可以（連結指到合作夥伴網站、YouTube、表單都行） | ❌ 只能追自己埋 tag 的網站 |

### 工作流程：兩邊怎麼串起來

```
① 短網址工具建 link + 填 UTM     ← 你在這裡定義規範
        ↓
② 把短網址貼到 EDM / 社群 / QR
        ↓
③ 使用者點擊
        ↓
  ┌──────────────────────────┐
  │ 短網址工具記錄 click      │  ← 點擊前資料：來源、裝置、地區、時間、variant
  │  （即時、跨任何 domain）   │
  └──────────────────────────┘
        ↓ 轉址 + 帶 UTM 參數
  ┌──────────────────────────┐
  │ Landing page + GA4       │  ← 點擊後資料：頁面行為、轉換、營收
  │  （讀 URL 上的 UTM 歸因）  │
  └──────────────────────────┘
        ↓
④ 兩邊用**同一組 utm_campaign** 對上
```

**關鍵**：兩邊用同一個 `utm_campaign` 值 → 在短網址工具看「哪個通路點最多」，在 GA4 看「哪個通路轉換最好」，合起來才是完整的 ROI 判斷。
...
|---|---|
| 這條短網址今天被點幾次？ | **短網址工具** |
| Facebook vs LinkedIn 哪個通路點擊率高？ | **短網址工具** |
| 紅色 CTA vs 藍色 CTA 誰贏？（A/B 變體點擊） | **短網址工具** |
| QR code 在哪個城市被掃最多？ | **短網址工具** |
| 連結是否還沒過期、是否要排程啟用？ | **短網址工具** |
| 訪客進站後看了幾頁才離開？ | **GA4** |
| 春季活動最後帶來多少註冊 / 下單？ | **GA4** |
| 某個 UTM campaign 的實際營收？ | **GA4**（接電商模組） |
| 用戶從 landing page 到結帳的漏斗轉換率？ | **GA4** |

### 常見誤解

- **「GA4 也能看點擊來源啊？」** → GA4 只能看「進到你網站」之後的來源。還沒進站（點了但跳離、載入失敗、去了第三方頁）你看不到。短網址工具看的是**原始點擊事件**本身。
- **「那我建連結時不填 UTM，GA4 也抓得到嗎？」** → 抓得到 referrer（粗略），但精細度遠不如 UTM。而且**跨不同 domain 的 campaign 對不起來**（同一場活動導去 A 網站和 B 網站時，GA 會視為兩筆獨立流量）。
- **「我只看點擊數就夠了吧？」** → 點擊 ≠ 轉換。點 1000 次但沒人下單的素材不值得投。所以還是要搭 GA4 看後段轉換。
